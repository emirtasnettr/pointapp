...
export function defaultCourierBirthDateValue(): string {
  const d = maxBirthDateForMinAge();
  d.setFullYear(d.getFullYear() - 7);
  return formatYmd(d);
}

export function maxBirthDateInputValue(): string {
  return formatYmd(maxBirthDateForMinAge());
}
