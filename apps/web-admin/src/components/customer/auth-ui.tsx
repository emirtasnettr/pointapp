...
export function AuthBrandPanel({
  ...
  return (
    <div
      className={cn(
        'relative flex h-full min-h-full flex-col justify-between overflow-hidden bg-brand p-8 text-white sm:p-10',
        className,
      )}
    >
      <div
...
export function AuthDemoHint({ children }: { children: ReactNode }) {
  return (
    <p className="rounded-2xl border border-zinc-200/90 bg-zinc-50 px-4 py-3 text-xs leading-relaxed text-zinc-600">
      {children}
    </p>
  );
}
