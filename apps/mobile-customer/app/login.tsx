...
export default function LoginScreen() {
  ...
  const onSubmit = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await apiPost<LoginResponse>('/auth/customer/login', { email: e, password });
...
const styles = StyleSheet.create({
  ...
  link: { marginTop: 26, alignItems: 'center', paddingVertical: 12 },
  linkTxt: { fontSize: 15, color: t.inkMuted, fontWeight: '500' },
  linkBold: { color: t.brand, fontWeight: '800' },
});
