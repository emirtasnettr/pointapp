...
<style>
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; margin: 0; padding: 16px; color: #1a2230; background: #fff; line-height: 1.55; font-size: 15px; }
  a { color: #16B24B; }
</style>
</head>
...
export function LegalDocumentModal({ slug, visible, onClose }: Props) {
  ...
  const load = useCallback(async (slugStr: string) => {
    ...
    setPage(null);
    try {
      const data = await apiGet<LegalPageResponse>(
...
const styles = StyleSheet.create({
  ...
  err: { color: t.danger, fontSize: 14, fontWeight: '600', textAlign: 'center' },
  footer: { padding: 16, paddingBottom: 24, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: t.border },
  closeBtn: { backgroundColor: t.brand, paddingVertical: 14, borderRadius: 12, alignItems: 'center' },
