...
import type { CourierAvailability } from '../../lib/courier-availability';
...
function poolTypeChip(type: string): { Icon: typeof Package; label: string } | null {
  switch (type) {
    case 'PACKAGE':
...
type ListResponse = { items: Delivery[]; total?: number };
...
export default function PoolTab() {
  ...
  const onAvailability = useCallback((s: CourierAvailability) => {
    setIsOnline(s.isOnline);
    setAvailLoaded(true);
  }, []);

  const load = useCallback(
    async (kind: 'initial' | 'pull' | 'refocus') => {
      if (!isOnline) {
        setItems([]);
        setErr(null);
        setLoading(false);
      ...
      setErr(null);
      try {
        const q = await apiGetAuth<ListResponse>('/courier/deliveries/pool');
        setItems(q.items);
      } catch (e) {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const kind = visitedRef.current && focusTick > 1 ? 'refocus' : 'initial';
      void load(kind);
    }, [availLoaded, isOnline, focusTick, load]),
  );

  return (
    <View style={styles.box}>
      <CourierOnlineToggle refreshKey={focusTick} onChange={onAvailability} />

      {loading && items.length === 0 && isOnline ? (
      ...
      <FlatList
        ...
        ListEmptyComponent={
          !loading && !err ? (
            <View style={styles.empty}>
              <Inbox color={t.inkSoft} size={32} strokeWidth={1.5} />
              {availLoaded && !isOnline ? (
                <>
                  <Text style={styles.emptyTitle}>Çevrimdışısınız</Text>
                  <Text style={styles.emptyTxt}>
                    Paketleri görmek için yukarıdan çevrimiçi olun.
                  </Text>
                </>
              ) : (
                <>
                  <Text style={styles.emptyTitle}>Havuz boş</Text>
                  <Text style={styles.emptyTxt}>Yeni işler burada listelenir.</Text>
                </>
              )}
            </View>
          ) : null
        ...
        renderItem={({ item }) => {
          const typeChip = poolTypeChip(item.type);
...
const styles = StyleSheet.create({
  ...
  loader: { marginTop: 20 },
  listContent: { paddingTop: 4, paddingBottom: 88 },
  errBox: { marginTop: 10 },
